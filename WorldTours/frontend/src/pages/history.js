...
function History() {
	...
	useEffect(() => {
		const getData = async () => {
            try {
				let response;
				response = await axios.get(`https://localhost:7276/booking/bookings?userId=${authUser.id}&isHistory=${true}`, {
                    headers: {
		...
		<div className="bookings narrow-conteiner">
			...
			<main className='vertical-list-page'>
				<div className='booking-list-filters-parameters'>
					...
					<div className='statistic-bookings-list'>
						<div><b>Всего поездок:</b> {bookings.length}</div>
						<div><b>Всего потрачено:</b> {bookings.reduce((sum, booking) => sum + booking.price, 0)} BYN</div>
